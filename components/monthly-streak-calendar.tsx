...
import { Calendar, Flame, Trophy, Target, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function MonthlyStreakCalendar({ studentId, className = "" }: MonthlyStreakCalendarProps) {
  ...
  const loadData = async () => {
    if (studentId) {
      try {
        ...
        if (activities.length > 0) {
          ...
          const monthsWithData = [...new Set(activities
            .filter(a => a.level > 0)
    ...
    setAvailableMonths([new Date(), new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)]);
  };
    ...
    for (let d = new Date(firstDay); d <= lastDay; d.setDate(d.getDate() + 1)) {
      ...
      if (hasActivity) {
        ...
        data.push({
          ...
          subjects: ['Math', 'Science', 'English'].slice(0, Math.floor(Math.random() * 2) + 1)
        });
      } else {
        data.push({
          date: dateStr,
  ...
  return (
    <Card className={`bg-gray-800/90 backdrop-blur-sm border-gray-600/50 p-4 ${className}`}>
      <div className="space-y-4">
        {/* Header */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Calendar className="h-4 w-4 text-green-400" />
            <span className="text-sm font-medium text-white">Learning Activity</span>
          </div>
            ...
            <div key={weekIndex} className="grid grid-cols-7 gap-1">
              {week.map((day, dayIndex) => (
                <div key={dayIndex} className="relative">
                  {day ? (
                    <motion.div
                      className={`w-8 h-8 rounded border cursor-pointer transition-all flex items-center justify-center ${getActivityColor(day.level)}`}
        ...
        <div className="flex items-center justify-between text-xs text-gray-500">
          ...
          <div className="flex gap-1">
            {[0, 1, 2, 3, 4].map((level) => (
              <div key={level} className={`w-3 h-3 rounded border ${getActivityColor(level)}`} />
            ))}
          </div>
          <span>More</span>
        </div>
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-3 text-xs">
              <div className="text-center">
                <div className="text-blue-400 font-bold">{hoveredDay.lessonsCompleted}</div>
                <div className="text-gray-400">Lessons</div>
              </div>
              <div className="text-center">
                <div className="text-green-400 font-bold">{hoveredDay.timeSpent}m</div>
                <div className="text-gray-400">Minutes</div>
              </div>
              <div className="text-center">
                <div className="text-purple-400 font-bold">{hoveredDay.quizzesCompleted}</div>
